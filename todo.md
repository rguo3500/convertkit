...
> 当前基线：网站审核已提交，European regulations 消息已发布，广告脚本和真实广告位仍关闭。


## AdSense 政策合规自查清单
- [x] 确定官方政策检查范围
- [x] 整理首页、法律页和广告合规项目
- [x] 结合 ConvertKit 当前状态标注证据
- [x] 交付详细清单与整改优先级

> 清单依据 Google AdSense 计划政策、资格要求、无效流量说明和 EU 用户同意政策整理；当前网站审核已提交，正式广告仍关闭。


## AdSense 自查清单执行
- [x] 核对生产版本与基础可访问性
- [x] 核验首页、法律页和运营者信息
- [x] 核验 CMP、Cookie、广告门控和 SEO
- [x] 汇总自查结果与审核后待办
- [x] 交付自查报告

> 本轮生产自动化自查全部通过；AdSense 审核、真实 client/slot 和授权后的 Network 验收仍待审核通过后执行。


## ads.txt 发布
- [x] 核对当前静态根文件与部署结构
- [x] 添加 ads.txt 并纳入构建产物
- [x] 验证文本格式、构建和生产路径
- [x] 交付部署与 AdSense 检查步骤

> `client/public/ads.txt` 已加入用户确认的单行记录；本地源文件和 `dist/public/ads.txt` 均验证通过。Cloudflare 发布后需确认线上根路径可访问。
