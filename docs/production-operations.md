...
生产域名每次发布后，至少抽查首页、一个 conversion pair、格式工具、批量转换页、`robots.txt` 和 `sitemap.xml`。weekly health workflow 同时使用 `lighthouserc.production.cjs` 对真实生产域名的五个核心页面运行 Lighthouse，并由 `scripts/compare-production-lighthouse.mjs` 与 `data/production-lighthouse-baseline.json` 比较 Performance、Accessibility、Best Practices 和 SEO；报告保存为 `artifacts/weekly-health/production-lighthouse-summary.md`。本地维护者可运行 `pnpm run audit:production` 手动复测，实验室分数应结合 Cloudflare RUM 趋势解释，而不应依据单次波动直接回滚。当前生产 robots.txt 包含 Cloudflare Managed content，已知 Lighthouse robots-txt 审计可能因此产生 `REVIEW_EXTERNAL`；应先在 Cloudflare Dashboard 检查 Managed robots 内容，再将仓库 Variable `ALLOW_CLOUDFLARE_MANAGED_ROBOTS` 设为 `false` 以恢复严格审计，或保持默认 `true` 并继续人工复核。只有在外部差异确认无误后，才建议设置 `FAIL_ON_PRODUCTION_LIGHTHOUSE_REGRESSION=true`。如果页面返回 200 但 Sitemap 仍包含旧域名，应优先检查 Cloudflare Pages 构建环境中的 `VITE_SITE_URL`，然后重新部署。
