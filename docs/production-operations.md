...
生产域名每次发布后，至少抽查首页、一个 conversion pair、格式工具、批量转换页、`robots.txt` 和 `sitemap.xml`。weekly health workflow 同时使用 `lighthouserc.production.cjs` 对真实生产域名的五个核心页面运行 Lighthouse，并由 `scripts/compare-production-lighthouse.mjs` 与 `data/production-lighthouse-baseline.json` 比较 Performance、Accessibility、Best Practices 和 SEO；报告保存为 `artifacts/weekly-health/production-lighthouse-summary.md`。本地维护者可运行 `pnpm run audit:production` 手动复测，实验室分数应结合 Cloudflare RUM 趋势解释，而不应依据单次波动直接回滚。如果页面返回 200 但 Sitemap 仍包含旧域名，应优先检查 Cloudflare Pages 构建环境中的 `VITE_SITE_URL`，然后重新部署。
