...
构建前会从 `data/conversion-pairs.json` 生成 registry、`client/public/sitemap.xml` 和 `client/public/robots.txt`。构建后应确认 Sitemap 中的 `<loc>` 均使用真实域名，不能保留 `lovexiaoyue.cc.cd` 占位地址。
...
将生产域名添加到搜索引擎站长工具后，提交 `https://lovexiaoyue.cc.cd/sitemap.xml`。域名、协议或路径变更时，重新设置 `VITE_SITE_URL` 并重新构建，以同步 canonical、Open Graph 和 Sitemap 地址。不要在本地预览域名上提交 Sitemap。
