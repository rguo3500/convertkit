...
const totalFailures = Object.values(categories).reduce((sum, count) => sum + count, 0);
const maxFailures = Math.max(1, ...Object.values(categories));
const esc = value => String(value ?? "").replace(/[&<>\"]/g, char => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" })[char]);
const rows = recent.map(run => `<tr><td>${esc(run.recordedAt)}</td><td class="${run.status === "delivered" ? "ok" : run.status === "failed" ? "bad" : "muted"}">${esc(run.status)}</td><td>${esc(run.failureCategory || "none")}</td><td>${Object.values(run.failureBuckets || {}).reduce((sum, count) => sum + Number(count || 0), 0)}</td></tr>`).join("\n");
const bars = Object.entries(categories).map(([category, count]) => `<div class="bar-row"><span>${esc(category)}</span><div class="bar-track"><div class="bar" style="width:${Math.round((count / maxFailures) * 100)}%"></div></div><strong>${count}</strong></div>`).join("\n") || '<p class="muted">No failures in the latest window.</p>';
const markdown = `# Webhook audit trend\n\nGenerated at ${new Date().toISOString()}. The report covers the latest ${recent.length} retained runs and contains no webhook URL or secret.\n\n| Metric | Value |\n| --- | ---: |\n| Runs in window | ${recent.length} |\n| Total failures | ${totalFailures} |\n| Latest status | ${recent.at(-1)?.status || "not configured"} |\n\n## Failure buckets\n\n| Category | Count |\n| --- | ---: |\n${Object.entries(categories).map(([category, count]) => `| ${category} | ${count} |`).join("\n") || "| none | 0 |"}\n\n## Recent runs\n\n| Recorded at (UTC) | Status | Failure category | Failure count |\n| --- | --- | --- | ---: |\n${recent.map(run => `| ${run.recordedAt || "unknown"} | ${run.status || "unknown"} | ${run.failureCategory || "none"} | ${Object.values(run.failureBuckets || {}).reduce((sum, count) => sum + Number(count || 0), 0)} |`).join("\n") || "| none | not configured | none | 0 |"}\n`;
